...
saveCropCanvas = function(canvas, tempId) {
...
getResults = function(){
    ...
    crop_output = $("#crop_output").get(0);
    //temp_ctx.scale(0.25, 0.25);

    frameOrdinal = cropFrames.push(temp_canvas);

    //thumbnail = capture(image, thumbnailScaleFactor);
    frameId = frameIndex = frameOrdinal - 1;
    $(temp_canvas).attr("data-frame-id", frameId);
    cropFrameRegistry[frameId] = temp_canvas;
    crop_output.appendChild(temp_canvas);
    $("#crop_output").sortable("refresh");
    //rescanCropThumbnails();
    //placeFrame(frameIndex, overlayClass);
    saveCropCanvas(temp_canvas, frameId);
